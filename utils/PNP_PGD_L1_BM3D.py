"""
The definition is about the use of BM3D denoising under the PNP-PGD-L1 framework
...
def PNP_PGD_L1_BM3D(mask, **PNP_PGD_L1_BM3D_opts):
    ...
    # Preparation
    # ----------------------------------------
    alpha = PNP_PGD_L1_BM3D_opts.get('alpha', 1)
    iter_num = PNP_PGD_L1_BM3D_opts.get('iter_num', 20)
    ...
    for idx, img in enumerate(L_paths):
        ...
        if n_channels == 1:
            img_H = img_H.squeeze()
        if save_E:
            util.imsave(img_E, os.path.join(E_path, img_name + 'PNP_PGD_L1_BM3D.png'))
